...
export const actions = {
    default: async ({ request }) => {
        const { legalIdType, legalIdNumber } = Object.fromEntries(await request.formData());
        ...
        const currentCustomerTicketPurchases = ticketPurchases.filter((ticketPurchase) => {
            ...
            const idMatches =
                ticketPurchaseData.customer.legalIdType == legalIdType &&
                ticketPurchaseData.customer.legalIdNumber == legalIdNumber;
        ...
        currentCustomerTicketPurchases.forEach(async (currentCustomerTicketPurchase) => {
            ...
            await sendConfirmationEmail(
                ...
                if (currentCustomerTicketPurchaseData.status == 'APPROVED') {
                    ticketPurchaseDao.updateStatusUsingGivenDocumentId(
                        currentCustomerTicketPurchaseRef,
                        'EMAIL_SENT'
                    );
                }
            });
